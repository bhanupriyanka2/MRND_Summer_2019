    ...
    for (int i = 0; i < MAXSIZE; i++) {
        for (int j = 0; j < MAXSIZE; j++) {
            if (sheet[i][j].string_data == NULL) {
                ...
                printf("%s ", sheet[i][j].string_data);
        }
...
char *TOPPER(char *sub, Cell **sheet) {
    ...
    char* result = NULL;
    for (int row = 1; row <  10; row++) {
        if (max < sheet[row][col_index].data) {
            max = sheet[row][col_index].data;
            result = sheet[row][0].string_data;
        }
    }
...
char *TOPPER_OF_CLASS(Cell **sheet){
	int max = -1;
	char *name = NULL;
	int col_index = 0;
	for (int row = 1; row < 10; row++){
		if (max < TOTAL(sheet[row][col_index].string_data, sheet)){
			max = TOTAL(sheet[row][col_index].string_data, sheet);
			name = sheet[row][col_index].string_data;
		}
	}
	return name;
}
...
int main() {
    ...
    while (1) {
        ...
        if (command -> type == 1)
			...
			char *name = !command->cell? TOPPER_OF_CLASS(sheet) : TOPPER(command->cell, sheet);
            if (name == NULL)
                printf("Name not found\n");
            else
                printf("%s\n", name);
